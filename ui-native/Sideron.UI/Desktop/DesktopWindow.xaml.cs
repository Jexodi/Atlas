...
public sealed partial class DesktopWindow : Window
{
    ...
    private readonly SideronUpdateService _updateService;
    private SideronUpdateConfiguration _updateConfiguration =
        SideronUpdateConfiguration.Default;

    private SideronUpdateManifest? _availableUpdateManifest;
    private SideronUpdateDownloadResult? _verifiedDownloadedUpdate;
    ...
    private void ApplySideronWindowIcon()
    {
        try
        {
            var candidates =
            ...
            )
            {
                ...
                UiLog.Info(
                    $"Sideron window icon loaded: {iconPath}");
            }
        }
        catch (Exception exception)
        {
            UiLog.Error(
                "Unable to apply Sideron window icon.",
                exception);
        }
    }
    ...
    private void ConfigureWindowBounds()
    {
        ...
        // Affiche Sideron dans la barre des tâches et dans les sélecteurs de
    ...
    {
        // The Sideron dock is intentionally independent from the Windows
        ...
        PointerRoutedEventArgs e)
    {
        ...
        if (
            IsPointerInsideSideronForegroundUi(
                source)
        )
        {
            return;
    ...
    private bool IsPointerInsideSideronForegroundUi(
        DependencyObject? source)
    {
        var current =
            source;
    ...
    private void SavePrimaryWindowLayouts()
    {
        ...
        {
            UiLog.Error(
                "Unable to save primary Sideron window layouts.",
                exception);
        }
    }
    ...
    private void UpdateSettingsHomeDashboard()
    {
        ...
        HomeListeningDetailText.Text =
            !_coreConnected
                ? "Core non connecté."
                : _currentListeningMode == "wake_word"
                    ? "Sideron attend le mot de réveil local avant d’ouvrir une session vocale."
                    : "Sideron maintient l’écoute vocale continue.";
    ...
    private void UpdateDisplaySettingsPresentation()
    {
        ...
        DisplaySettingsAppliedText.Text =
            isApplied
                ? "Cet écran héberge actuellement le bureau Sideron."
                : $"Sideron est actuellement sur {(_atlasDisplay is null ? "un autre écran" : BuildDisplayLabel(_atlasDisplay))}.";
        ...
        RoutedEventArgs e)
    {
        ...
        UiLog.Info(
            "Sideron primary window layout reset to defaults.");
    }
        ...
        RoutedEventArgs e)
    {
        ...
        UiLog.Info(
            "Sideron widget layout reset to right preset.");
    }
        ...
        RoutedEventArgs e)
    {
        ...
        UiLog.Info(
            "Sideron desktop layout reset to defaults.");
    }
        ...
        string wakeWord)
    {
        ...
        if (!connected)
        {
            ...
            ListeningStateDescriptionText.Text =
                "Démarre Sideron Core pour utiliser les réglages de voix.";
        ...
        if (continuous)
        {
            ...
            ListeningStateDescriptionText.Text =
                "Sideron écoute en permanence et peut répondre sans mot de réveil.";
            ...
            VoiceWidgetModeText.Text =
                "Sideron écoute sans mot de réveil.";
    ...
    private async Task CheckForUpdatesAsync()
    {
        ...
        try
        {
            ...
            if (
                (result.Status == SideronUpdateStatus.UpdateAvailable
                    || result.Status == SideronUpdateStatus.ReinstallAvailable)
            ...
            )
            {
                ...
                DownloadUpdateButton.IsEnabled =
                    downloadConfigured;
                DownloadUpdateButton.Content = result.Status == SideronUpdateStatus.ReinstallAvailable
                    ? "Télécharger pour réinstaller la Release" : "Télécharger la mise à jour";
                if (downloadConfigured && result.Status == SideronUpdateStatus.UpdateAvailable)
        ...
        catch (HttpRequestException exception)
        {
            ...
            UiLog.Error(
                "Sideron update HTTP check failed.",
                exception);
        }
        catch (TaskCanceledException exception)
        {
            ...
            UiLog.Error(
                "Sideron update check timed out.",
                exception);
        }
        catch (Exception exception)
        {
    ...
    private async Task NotifyUpdateAvailableAsync(SideronUpdateManifest manifest, string channel)
    {
        ...
        try
        {
            ...
            {
                Title = "Mise à jour Sideron disponible",
                Content = $"Sideron {manifest.Version} est disponible sur le canal {channel}. Voulez-vous ouvrir les mises à jour ?",
        ...
        RoutedEventArgs e)
    {
        ...
        {
            UpdateStatusText.Text =
                "Vérifiez d'abord les mises à jour Sideron.";
        ...
        UpdateStatusText.Text =
            $"Téléchargement d'Sideron {manifest.Version}…";
        ...
        try
        {
            ...
            UpdateStatusText.Text =
                $"Sideron {manifest.Version} téléchargé et SHA-256 validé.";
            ...
            UiLog.Info(
                $"Sideron update downloaded and verified: version={manifest.Version}, path={result.FilePath}, sha256={result.Sha256}, size={result.SizeBytes}");
        }
        catch (InvalidDataException exception)
        {
            ...
            UiLog.Error(
                "Sideron update integrity validation failed.",
                exception);
        }
        catch (HttpRequestException exception)
        {
            ...
            UiLog.Error(
                "Sideron update download HTTP request failed.",
                exception);
        }
        catch (TaskCanceledException exception)
        {
            ...
            UiLog.Error(
                "Sideron update download timed out.",
                exception);
        }
        catch (Exception exception)
        {
            ...
            UiLog.Error(
                "Sideron update download failed.",
                exception);
        }
        finally
        {
        ...
        RoutedEventArgs e)
    {
        ...
        {
            UpdateStatusText.Text =
                "Téléchargez d'abord une mise à jour Sideron vérifiée.";
            return;
        }
        ...
        try
        {
            ...
            var dialog =
                new ContentDialog
                {
                    Title = $"Installer Sideron {manifest.Version} ?",
                    Content =
                        "Sideron va demander une autorisation administrateur, installer la mise à jour en arrière-plan "
            ...
            UpdateStatusText.Text =
                $"Préparation de la mise à jour Sideron {manifest.Version}…";
            ...
            var startInfo =
                new System.Diagnostics.ProcessStartInfo
                {
                    ...
                    Verb = "runas",
                    // Ne jamais transmettre C:\Program Files\SIDERON comme
            ...
            // toutefois attendre la fin réelle d'SIDERON.exe avant de toucher
            ...
            if (process is null)
            {
                throw new InvalidOperationException(
                    "Windows n'a pas pu lancer l'installateur Sideron.");
            }

            UiLog.Info(
                $"Verified Sideron installer launched: version={manifest.Version}, path={verified.FilePath}, sha256={verified.Sha256}");

            await ShutdownSideronForUpdateAsync();
            Close();
        }
        catch (System.ComponentModel.Win32Exception exception)
        {
            if (exception.NativeErrorCode == 1223)
            {
                ...
                UiLog.Info(
                    "Sideron silent update elevation canceled by user.");
            }
            else
            {
                UpdateStatusText.Text =
                ...
                UiLog.Error(
                    "Unable to launch Sideron silent update.",
                    exception);
            }
        }
        catch (InvalidDataException exception)
        {
            ...
            UiLog.Error(
                "Sideron update final integrity validation failed.",
                exception);
        }
        catch (Exception exception)
        {
            ...
            UiLog.Error(
                "Sideron silent update launch failed.",
                exception);
        }
        finally
        {
    ...
    private async Task ShutdownSideronForUpdateAsync()
    {
        ...
        {
            UiLog.Error(
                "Sideron Core shutdown before update installer failed.",
                exception);
        ...
        switch (section)
        {
            case "display":
                ...
                SettingsPageSubtitleText.Text =
                    "Configuration de l’écran utilisé par Sideron";
                break;
            ...
            case "desktop":
                SettingsPageTitleText.Text =
                    "Bureau";
                SettingsPageSubtitleText.Text =
                    "Disposition des fenêtres et widgets Sideron";
                break;
            ...
            case "permissions":
                SettingsPageTitleText.Text =
                    "Permissions";
                SettingsPageSubtitleText.Text =
                    "Contrôle des actions autorisées au Core Sideron";
                break;
            ...
            case "updates":
                SettingsPageTitleText.Text =
                    "Mises à jour";
                SettingsPageSubtitleText.Text =
                    "Version, canal et disponibilité des nouvelles versions Sideron";
                break;
            ...
            case "about":
                SettingsPageTitleText.Text =
                    "À propos";
                SettingsPageSubtitleText.Text =
                    "Informations sur l’interface Sideron";
                break;
            ...
            default:
                SettingsPageTitleText.Text =
                    "Accueil";
                SettingsPageSubtitleText.Text =
                    "Vue d’ensemble des paramètres Sideron";
                break;
        }
    ...
    private void UpdateStartupRegistrationPresentation()
    {
        ...
        StartupRegistrationCommandText.Text =
            ...
            && !string.IsNullOrWhiteSpace(
                status.Command)
                ? status.Command
                : "Aucun lancement automatique Sideron enregistré dans Windows.";
    }
        ...
        string mode)
    {
        ...
        switch (mode)
        {
            case "restricted":
                description =
                    "Mode le plus prudent : Sideron peut consulter et exécuter uniquement les actions considérées comme sûres.";
            ...
            case "advanced":
                description =
                    "Sideron peut effectuer les modifications locales directement et demande confirmation pour les actions administrateur.";
    ...
    private void ApplySideronDialogStyle(
        ContentDialog dialog)
    {
        ...
        // Keep the Sideron desktop visible instead of heavily dimming it.
        dialog.Resources[
        ...
        string path)
    {
        ...
        var button =
            new Button
            {
                Style =
                    Root.Resources[
                        "SideronExplorerEntryButtonStyle"]
                    as Style,
                Width = 146,
                Height = 108,
                Padding =
    ...
    private void Sideron_Click(
        object sender,
        RoutedEventArgs e)
    {
        UpdateCoreDockControlUi();

        SideronCoreFlyout.ShowAt(
            SideronCoreButton);
    }

    private async void SideronTestCore_Click(
        object sender,
        RoutedEventArgs e)
    {
        if (!_ipc.IsConnected)
        ...
        {
            UiLog.Error(
                "Sideron Core connection test failed.",
                exception);
    ...
    private async void SideronStartCore_Click(
        object sender,
        RoutedEventArgs e)
    {
        if (_ipc.IsConnected)
        ...
        {
            UiLog.Error(
                "Unable to start Sideron Core from dock.",
                exception);
    ...
    private async void SideronStopCore_Click(
        object sender,
        RoutedEventArgs e)
    {
        ...
        {
            UiLog.Error(
                "Unable to stop Sideron Core from dock.",
                exception);
    ...
    private string GetExplorerTabDisplayName(string directory)
    {
        try
        {
            ...
            return string.IsNullOrWhiteSpace(name) ? "SIDERON" : name;
        }
        catch
        {
            return "SIDERON";
    ...
    private async void ExitSideron_Click(
        object sender,
        RoutedEventArgs e)
    {
        var dialog =
            new ContentDialog
            {
                Title = "Quitter Sideron",
                Content =
                    "Voulez-vous fermer Sideron Desktop ?",
        ...
        {
            UiLog.Error(
                "Sideron Core graceful shutdown failed; UI will still close.",
                exception);
        ...
        {
            // Toujours fermer Sideron Desktop, même si le Core ou l'IPC
            // ne répondent plus.
            Close();
        }
    }
    ...
    private sealed class SecondaryExplorerWindow
    {
        ...
        private async Task DeleteSelectedAsync()
        {
            ...
            _owner.ApplySideronDialogStyle(
                dialog);
