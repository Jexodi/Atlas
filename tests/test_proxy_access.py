...
class AccessTests(unittest.IsolatedAsyncioTestCase):
    async def test_no_key_and_no_activation_fails_closed(self):
        with patch.dict(os.environ, {}, clear=True):
            with self.assertRaisesRegex(RuntimeError, 'non activé'):
                create_client()

    async def test_activation_preferred_over_owner_key(self):
        with tempfile.TemporaryDirectory() as tmp:
            path = Path(tmp) / 'access.bin'
            path.touch()
            with patch('atlas.ai.access.access_path', return_value=path), \
                 patch('atlas.ai.access.decrypt_access', return_value='atlas_test_fixture'), \
                 patch.dict(os.environ, {'OPENAI_API_KEY': 'owner-test-key'}):
                client = create_client()
                self.assertEqual(client.api_key, 'atlas_test_fixture')
