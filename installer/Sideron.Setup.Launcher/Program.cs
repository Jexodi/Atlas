...
namespace SIDERON.Setup;

internal static class Program
{
    private const string PayloadResourceName = "SideronSetup.Payload.zip";
    private const string IconResourceName = "SideronSetup.Icon.ico";
    private const string SetupMutexName = @"Global\SIDERON.Setup.Launcher";
    ...
    private static int Main(string[] args)
    {
        ...
        // SIDERON.exe est installé dans C:\Program Files\SIDERON et peut lancer
        // ce programme avec ce dossier comme répertoire courant. Même après
        // la fermeture d'SIDERON.exe, Windows refuse alors de renommer le
        ...
        try
        {
            ...
            if (!ownsSetupMutex)
            {
                ...
                {
                    ShowError(
                        "Une autre fenêtre d'installation Sideron est déjà ouverte.");
                }
            ...
            if (silentUpdate)
            {
                        ...
                        silentInstallerScript))
                {
                    throw new FileNotFoundException(
                        "Les fichiers internes de mise à jour Sideron sont incomplets.",
                        silentInstallerScript);
                }
                ...
                _ = Task.Run(
                    () =>
                    {
                        try
                        {
                            ...
                            {
                                throw new FileNotFoundException(
                                    "Les fichiers internes de l'installateur Sideron sont incomplets.",
                                    installerScript);
                            }
                            ...
                            if (installerProcess is null)
                            {
                                throw new InvalidOperationException(
                                    "Impossible de démarrer l'installateur Sideron.");
                            }
                            ...
                            if (signal == 1)
                            {
                                ...
                                {
                                    throw new InvalidOperationException(
                                        "L'interface de l'installateur Sideron n'a pas pu démarrer.");
                                }

                                throw new InvalidOperationException(
                                    "L'interface de l'installateur Sideron s'est fermée avant son affichage.");
                            }
                        }
        ...
        catch (Exception exception)
        {
            ...
            {
                ShowError(
                    "Sideron Setup n'a pas pu démarrer.\n\n"
                    + exception.Message);
            }
        ...
        string progressFile)
    {
        ...
        {
            throw new FileNotFoundException(
                "L'interface de progression Sideron.UpdateHost.exe est introuvable.",
                updateHostPath);
        }
        ...
        {
            throw new InvalidOperationException(
                "Impossible de démarrer Sideron.UpdateHost.");
        }
        ...
        bool restartSideron)
    {
        ...
        {
            throw new InvalidOperationException(
                "Impossible de démarrer le moteur de mise à jour Sideron.");
        }
    ...
    private static void WaitForSideronLauncherExit(
        int? processId)
    {
        if (
            processId is null
        ...
        try
        {
            ...
            )
            {
                throw new TimeoutException(
                    "SIDERON.exe ne s'est pas arrêté dans le délai prévu.");
            }
        }
    ...
    private static Form CreateSplashWindow()
    {
        ...
        var status =
            ...
            {
                Dock = DockStyle.Fill,
                Text = "Initialisation des composants Sideron…",
    ...
    private static void FitFormToCurrentScreen(Form form)
    {
    ...
    private static Icon? LoadEmbeddedSideronIcon()
    {
        Assembly assembly =
    ...
    private static void ExtractEmbeddedPayload(string destination)
    {
        ...
        {
            throw new InvalidOperationException(
                "Le payload interne Sideron est introuvable.");
        }
        ...
        foreach (ZipArchiveEntry entry in archive.Entries)
        {
                    ...
                    StringComparison.OrdinalIgnoreCase))
            {
                throw new InvalidDataException(
                    "Le payload Sideron contient un chemin non autorisé.");
            }
