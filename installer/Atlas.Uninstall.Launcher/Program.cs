    ...
    private static int Main(string[] args)
    {
        ...
        try
        {
            ...
            string sourceScript = Path.Combine(
                installRoot, "installer", "uninstall_atlas.ps1");

            if (!File.Exists(sourceScript))
            {
                AtlasMessageForm.ShowError(
                    "Le programme de désinstallation Atlas est incomplet.");
                return 2;
            }

            string temporaryDirectory = Path.Combine(
                Path.GetTempPath(), "AtlasUninstall", Guid.NewGuid().ToString("N"));
            Directory.CreateDirectory(temporaryDirectory);

            string temporaryScript = Path.Combine(
                temporaryDirectory, "uninstall_atlas.ps1");
            ...
            {
                AtlasMessageForm.ShowError(
                    "Impossible de démarrer la désinstallation Atlas.");
                return 3;
            }
        ...
        {
            AtlasMessageForm.ShowError(
                "La désinstallation Atlas n'a pas pu démarrer.\n\n" +
                exception.Message);
            return 1;
        }
    }
...
internal sealed class UninstallChoiceForm : Form
{
    ...
    public UninstallChoiceForm()
    {
        ...
        var title = CreateLabel(
            "DÉSINSTALLATION ATLAS", 16F, FontStyle.Bold,
        ...
        var description = CreateLabel(
            "Atlas va être retiré de cet ordinateur. Vous pouvez conserver vos fichiers ou supprimer toutes les données gérées par Atlas.",
        ...
        var removeButton = CreateChoiceButton(
            ...
            "Supprime l'application, le service et toutes les données Atlas. Cette action est irréversible.",
            true);
...
internal sealed class AtlasMessageForm : Form
{
    private AtlasMessageForm(string message)
    {
        Text = "Atlas";
        StartPosition = FormStartPosition.CenterScreen;
    ...
    {
        using var form = new AtlasMessageForm(message);
        form.ShowDialog();
    }
